...
const SaveRobotDetails = () => {
  const [robotDetails, setRobotDetails] = useState({
    ...
    serialNumber: "",
    IPAddress: "",
    image: "", // Ensure this is set correctly
    status: "active",
    location: "",
    subLocation: "",
    robotInitializeDate: "",
  ...
  const fetchLocationNames = async (email) => {
   ...
   const fetchSubLocations = async (email, locationName) => {
    try {
      const response = await axios.get(`/user/${email}/location/${locationName}/sublocations`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
  ...
  const handleChange = (e) => {
    ...
    if (name === "emailId") {
      fetchUserName(value);
      fetchLocationNames(value); // Fetch locations when email changes
      setRobotDetails((prevState) => ({
        ...prevState,
        emailId: value,
        location: "", // Reset location when email changes
        subLocation: "", // Reset sub-location when email changes
      }));
    } else if (name === "location") {
      fetchSubLocations(robotDetails.emailId, value); // Fetch sub-locations when location changes
    } else if (name === "model") {
      fetchModelImage(value); // Fetch image when the model changes
    }
